 ...
 */
import { ParasutApiError, ParasutAuthError, ParasutNotFoundError, ParasutRateLimitError, ParasutValidationError, ParasutNetworkError, } from '@yigitkonur/parasut-node-sdk';
/**
...
function handleValidationError(error, context) {
    ...
    parts.push('', 'Fix: Check the parameter values and try again.', 'Common issues:', '- Required fields are missing', '- Values are in wrong format (e.g., date should be YYYY-MM-DD)', '- Referenced IDs (contact, product) do not exist');
    return {
        content: [{ type: 'text', text: parts.join('\n') }],
        isError: true,
    };
